...
- **Zero runtime dependencies.** `koffi` (embedded transport), `undici` (in-process intercept),
  and `vitest` (Vitest testkit) are *optional* peer/optional dependencies, pulled in only if you
  use that feature.
...
No Docker, no child process, OS-assigned ports. Requires the optional `koffi` peer dependency:
`npm i -D koffi`. See [`examples/quickstart-embedded.ts`](examples/quickstart-embedded.ts).
