...
/** Real probe for the no-`transport` default: true when the `@rift-vs/rift-embedded` package
 * (the embedded transport since the #39 split) resolves. This only answers "is the package there",
 * not "will it actually load a cdylib" (koffi's native module may still fail) — a `false` here just
 * steers the default toward `spawn` instead; a wrong `true` still surfaces as a clear
 * `EngineUnavailable` below. */
