...
describe('package.json bootstrap invariants', () => {
  ...
  it('points repository metadata at the extracted rift-node repo and its workspace directory', () => {
    expect(pkg.repository?.url).toContain('rift-node');
    // Monorepo since #39: the package lives in a workspace subdirectory, and npm provenance /
    // repo deep-links rely on `repository.directory` naming it.
    expect(pkg.repository?.directory).toBe('packages/rift-core');
